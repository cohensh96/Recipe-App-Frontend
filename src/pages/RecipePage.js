...
const RecipePage = () => {
    ...
    {!loading && !error && data !== null && 
      <article className='w-3/4 mx-auto bg-white rounded-lg shadow-md p-6'>
        <div className='relative'>
          <img src={getImage(data.Image)} alt={data.recipeName} className='h-64 w-full object-cover rounded-t-lg' />
       ...
       <section className='px-4 py-3'>
          <p className=" font-bold text-gray-900 pt-2 mb-3">Author:{data.author}</p>
            <p className='text-gray-600'><b>Callories:</b> {data.recipeCallories}</p>
            <div className="w-full flex items-center">
            <FaClock className="mr-2 text-black" />
            <p className='text-gray-600'><b>Cooking time: </b>{data.recipeTime}</p>
            </div>
          </section>
          <section className='px-4 py-3 bg-gray-100'>
            {!commentsloading && !commentserror && comments !== null && <RatingSection comments={comments}/>}
          </section>
          <section className='flex flex-col'>
            <div>
              <h3 className="mt-5 font-bold">Ingredients:</h3>
              <ul className='flex flex-col list-inside hover:list-inside
              list-disc list-inside text-slate-700 bg-white ring-1 ring-slate-900/5 p-4 pl-8 '>
                {data && data.recipeIngredients && (data.recipeIngredients).map((ingrdeent, index) => 
            ...
            <div>
              ...
              list-disc text-slate-700 bg-white ring-1 ring-slate-900/5 p-4 pl-8 '>
                {data && data.recipeCategorys && (data.recipeCategorys).map((category, index) => 
  ...
  <h3 className="mb-4 w-3/4 mx-auto text-lg font-semibold text-gray-900">Comments</h3>
  {commentsloading && <p>Loading Comments ..</p>}
    {!commentsloading && commentserror && <p>{commentserror}</p>}
    {!commentsloading && !commentserror && comments !== null && 
      <article>
        {comments.map((singleComment,index) => (
          <section key={singleComment._id} className={`${index > 0 ? "mt-4" : ""} w-3/4 mx-auto border rounded-lg px-4 py-2 sm:px-6 sm:py-4 bg-gray-100`}>
            <h3 className="text-xl font-bold mb-2 text-amber-500">{singleComment.author}</h3>
            {isEdit && EditSection === singleComment._id ? 
              <div>
                <p><input className='text-gray-800 mb-4 rounded-md bg-gray-300 border border-3 focus:border-orange-500 focus:outline-none' type="text" placeholder={`${singleComment.comment}`} value={editComment} onChange={(e) => setEditComment(e.target.value)} name="" id="" /></p>
                <p className='flex justify-start'>Rating:
              ...
              <div>
                  ...
                  <p className="text-gray-700">Rating: {singleComment.rating} </p>
              </div>
            }
            {!loadinguser && !erroruser && singleComment.author === user.username && 
            <article className='flex ml-auto flex-col items-end justify-evenly'>
              {isEdit && EditSection === singleComment._id &&
                <button className='flex mb-5 bg-lime-300 border-2 border-green-400 rounded-md px-3 py-1.5 p-auto items-center justify-center hover:bg-green-500'
                onClick={() => {handleSaveChanges(singleComment)}}>
                  <BiEdit className=' text-lg mr-2'/>
                  Save changes
                </button>}
                <button className='flex bg-amber-300 border-2 border-amber-400 rounded-md px-3 py-1.5 p-auto items-center justify-center hover:bg-yellow-500'
                onClick={() => {
                ...
                <button className='flex mt-5 bg-rose-300 border-2 border-rose-400 rounded-md px-3 py-1.5 p-auto items-center justify-center hover:bg-red-500'
